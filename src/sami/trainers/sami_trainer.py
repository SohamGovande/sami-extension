...
def track_optimizer_updates(optimizer, model, scheduler):
    updated_params = []
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            update_magnitude = torch.norm(param.data - original_params[name])
            if update_magnitude > 0:
                updated_params.append((name, f"{update_magnitude:.6f}"))
    
    return updated_params

def are_parameters_updated(param_values_orig, param_values_new):
    """Compares original and new parameter values to check if they have been updated."""
    print("OLD PARAMETERS", param_values_orig[:10])
    print("NEW PARAMETERS", param_values_new[:10])
    differences = [np.linalg.norm(new - orig) for orig, new in zip(param_values_orig, param_values_new)]
...
def prepare_logits_labels(
    ...
    mask = _get_mask_without_masking_eos(
...
class SAMITrainer:
    def __init__(
        ...
        # pick 5 random layers from model 
        self.random_layer_names = random.sample(list(model.state_dict().keys()), 5)
        self.random_layer_names = [layer_name for layer_name in self.random_layer_names if 'model.layers' in layer_name]
        # initial random layer weights 
        self.initial_random_layer_weights = {layer_name: model.state_dict()[layer_name].clone().float().cpu().numpy() for layer_name in self.random_layer_names}
        ...
        def lr_lambda(current_step: int):
            return_val = config.training.lr
            if current_step % 100 == 0:
                print(f"lr: {return_val}, step: {current_step}")
            # if current_step < 1000:
            #     return_val = config.training.lr
            # else:
            #     return_val = config.training.lr * (0.1 ** (current_step / config.training.total_steps))
            # if wandb.run:
            #     wandb.log({"lr": return_val})
            return return_val
        ...
        for param_group in self.optimizer.param_groups:
            for param in param_group['params']:
                if not param.requires_grad:
                    print("This parameter is in the optimizer but is frozen:", param)
        dist.barrier()
    ...
    def save_model(
        ...
        six_random_chars = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
        checkpoint_dir = os.path.join(self.config.training.checkpoint_dir, f"epoch-{epoch}-{six_random_chars}")
        os.makedirs(checkpoint_dir, exist_ok=True)
        state_random_layers = {layer_name: state[layer_name].clone().float().cpu().numpy() for layer_name in self.random_layer_names}
        every_layer_same = True
        for layer_name in self.random_layer_names:
            print(state_random_layers[layer_name])
            print(self.initial_random_layer_weights[layer_name])
            if not np.array_equal(state_random_layers[layer_name], self.initial_random_layer_weights[layer_name]):
                every_layer_same = False
                break
        if every_layer_same:
            raise ValueError("Random layers have not changed.")
        else:
            print("Confirming random layers have changed.")
        
        if self.config.training.save_option == "hf": # inefficient but stores HF format directly? 
            save_model = AutoModelForCausalLM.from_pretrained(**self.config.model.model_config)
            save_model.load_state_dict(state)
            save_model.save_pretrained(checkpoint_dir)
            self.tokenizer.save_pretrained(checkpoint_dir)
            self.last_saved_model = checkpoint_dir
    ...
    def compute_metrics(
        ...
        if torch.isnan(logits).any():
            print(f"454 NaN logits at {self.last_saved_model}")
        batch_logprobs = get_batch_logprobs(logits, labels)
        if torch.isnan(batch_logprobs).any():
            print(f"457 NaN batch logprobs at {self.last_saved_model}")
        ...
        if torch.isnan(loss).any():
            print(f"469 NaN loss at {self.last_saved_model}")
        ...
        for epoch in range(self.config.training.n_epochs):
            ...
            for step, batch in tqdm.tqdm(enumerate(self.train_dataloader), desc=f"Running epoch: {epoch}"):
                ...
                if (step + 1) % self.config.training.gradient_accumulation_steps == 0 or (step + 1) == len(self.train_dataloader):
                    if DEBUG_MODE:
                        params = lambda: self.model.named_parameters()

                        param_values_orig = []
                        for name, param in params():
                            print(f"Gradient norm for {name}: {param.grad.norm()}")
                            param_values_orig.append(param.data.clone().float().cpu().numpy())

                    self.model.clip_grad_norm_(self.config.training.max_grad_norm)
                    if DEBUG_MODE:
                        updated_params = track_optimizer_updates(self.optimizer, self.model, self.scheduler)
                    else:
                        self.optimizer.step()
                        if self.scheduler:
                            self.scheduler.step()
                    ...
                    if DEBUG_MODE:
                        param_values_new = []
                        for name, param in params():
                            param_values_new.append(param.data.clone().float().cpu().numpy())
                        if self.local_rank == 0:
                            print(f"Step {step}, Updated parameters and their magnitudes:")
                            for name, magnitude in updated_params:
                                print(f"  {name}: {magnitude}")
                            
                            total_difference = are_parameters_updated(param_values_orig, param_values_new)
                            if total_difference > 0:
                                print("Parameters updated w/ total diff", total_difference)
                            else:
                ...
                if self.local_rank == 0:
                    ...
                    if torch.isnan(reduced_loss) or torch.isnan(raw_reduced_loss):
                        with open(f"batch_epoch{epoch}_step{step}.pkl", "wb") as f:
                            import pickle
                            pickle.dump({k: v.to('cpu') for k, v in batch.items()}, f)
